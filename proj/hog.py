...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    diff = abs(player_score % 10 - opponent_score // 10 % 10)
    if diff == 0:
        return 1
    else:
        return diff * 3
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # "dice" parameter needs to be explicitly passed to cater to custom dice argument
    # END PROBLEM 4
...
def always_roll(n):
    ...
    def player_strategy(score, opponent_score):
        return n
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while i <= 10:
        average_score = make_averaged(roll_dice, times_called)(
            i, dice
        )  # average score of i dice rolls called times_called times
        if average_score > highest_score_average:
            highest_score_average = average_score
            max_scoring_num_rolls = i
        i += 1
        if i == 11:  # out of range cessation
            break
